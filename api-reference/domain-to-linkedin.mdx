...
<Note>
A successful lookup costs **0.5 credits**, billed asynchronously. No-result and failed lookups cost zero. Billing delays, outages, and insufficient credits do not block the result; an outstanding charge is retried later. The limit is **60 requests per minute per workspace**.
</Note>
