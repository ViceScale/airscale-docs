...
const SOURCE_SHA = "4e25a163dc94bd8ee34431b164670e9811a013ee";
...
test("Post engagement operations model synchronous cursor pagination and enrichment billing", () => {
  assert.equal(postEngagementOperations.length, 2);
  for (const entry of postEngagementOperations) {
    const operation = entry.operation;
    ...
    assert.deepEqual(operation.parameters, [{
      ...
      in: "header",
      required: true,
      description: "A UUID generated by your application. Reuse it with the same request body when retrying a lost response.",
    ...
    assert.deepEqual(request.properties.limit, {
      type: "integer",
      minimum: 1,
      maximum: 25,
      default: 25,
    ...
    assert.deepEqual(request.properties.cursor, {
      type: ["string", "null"],
      minLength: 1,
      maxLength: 8192,
    ...
    assert.deepEqual(response.examples.page.value.items[0], {
      ...
      headline: "Example role at Example Company",
      reaction_type: entry.path.endsWith("likers") ? "LIKE" : "COMMENT",
      comment: entry.path.endsWith("likers") ? null : "Example comment",
      post_url: "https://www.linkedin.com/posts/example_activity-7376356221991178240"
    });
    assertUnauthorizedReference(operation);
  }
});
