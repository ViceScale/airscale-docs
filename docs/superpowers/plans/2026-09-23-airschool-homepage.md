...
- [x] Regenerate agent discovery files using `npm run agents:build`; run `npm run validate` and generate/validate staging and production artifacts.
- [x] Run the homepage in Mintlify; verify desktop, mobile, dark mode, search click/keyboard/results, corrected shortcut, API card navigation, image loading, and browser errors. Record exact evidence.
- [x] Commit/review the source change, publish only the authorized noindex staging artifact, verify hosted content/SEO, and refresh the prepared production artifact without selecting it or changing DNS.
